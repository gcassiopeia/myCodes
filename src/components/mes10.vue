    ...
    <div>
      ...
      <table id="table-3">
        ...
        <tr v-for="item in tableInfo">
          ...
          <td>{{item.waterLevel ? item.waterLevel : ''}}</td>
          <td>{{item.containWater ? item.containWater : ''}}</td>
          <td>{{item.waterYield ? item.waterYield : ''}}</td>
...
//          console.log(error,1)
//        })
      this.$ajax.post(this.server +':8084/date/select?areaName=文水县&contentName=县域地下水资源质量',{}).then(res =>{
