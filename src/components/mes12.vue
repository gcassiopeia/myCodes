    ...
    <div>
      ...
      <table id="table-3">
        ...
        <tr v-for="item in tableInfo">
          ...
          <td>{{item.property ? item.property : ''}}</td>
          <td>{{item.e ? item.e : ''}}</td>
          <td>{{item.f ? item.f : ''}}</td>
          <td>{{item.coverAreaReason ? item.coverAreaReason : ''}}</td>
          <td>{{item.area ? item.area : ''}}</td>
          <td>{{item.abandonTime ? item.abandonTime : ''}}</td>
          <td>{{item.removeState ? item.removeState : ''}}</td>
          <td>{{item.recoverState ? item.recoverState : ''}}</td>
          <td>{{item.recoverArea ? item.recoverArea : ''}}</td>
          <td>{{item.unRecoverArea ? item.unRecoverArea : ''}}</td>
...
//          console.log(error,1)
//        })
      this.$ajax.post(this.server +':8084/date/select?areaName=文水县&contentName=废弃场地调查',{}).then(res =>{
