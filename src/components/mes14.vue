    ...
    <div>
      ...
      <table id="table-3">
        ...
        <tr v-for="item in tableInfo">
          <td>{{item.sinkArea ? item.sinkArea : ''}}</td>
          <td>{{item.e ? item.e : ''}}</td>
          <td>{{item.f ? item.f : ''}}</td>
          <td>{{item.area ? item.area : ''}}</td>
          <td>{{item.place ? item.place : ''}}</td>
          <td>{{item.waterDepth ? item.waterDepth : ''}}</td>
          <td>{{item.groundType ? item.groundType : ''}}</td>
          <td>{{item.occurTime ? item.occurTime : ''}}</td>
          <td>{{item.occurReason ? item.occurReason : ''}}</td>
...
//          console.log(error,1)
//        })
      this.$ajax.post(this.server +':8084/date/select?areaName=文水县&contentName=地面下陷',{}).then(res =>{
