    ...
    <div>
      ...
      <table id="table-3">
        ...
        <tr v-for="item in tableInfo">
          ...
          <td>{{item.checkIndex ? item.checkIndex : ''}}</td>
          <td>{{item.dayAverage ? item.dayAverage : ''}}</td>
          <td>{{item.yearPotency ? item.yearPotency : ''}}</td>
          <td>{{item.aWeather ? item.aWeather : ''}}</td>
          <td>{{item.bWeather ? item.bWeather : ''}}</td>
          <td>{{item.mainPollute ? item.mainPollute : ''}}</td>
        </tr>
      </table>
...
//          console.log(error,1)
//        })
      this.$ajax.post(this.server +':8084/date/select?areaName=文水县&contentName=县域大气环境',{}).then(res =>{
