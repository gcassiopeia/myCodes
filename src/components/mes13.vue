    ...
    <div>
      ...
      <table id="table-3">
        ...
        <tr v-for="item in tableInfo">
          ...
          <td>{{item.occupyReason ? item.occupyReason : ''}}</td>
          <td>{{item.e ? item.e : ''}}</td>
          <td>{{item.f ? item.f : ''}}</td>
          <td>{{item.occupyLength ? item.occupyLength : ''}}</td>
          <td>{{item.occupyWidth ? item.occupyWidth : ''}}</td>
          <td>{{item.occupyArea ? item.occupyArea : ''}}</td>
          <td>{{item.landType ? item.landType : ''}}</td>
          <td>{{item.protectMethod ? item.protectMethod : ''}}</td>
          <td>{{item.projectNum ? item.projectNum : ''}}</td>
...
//          console.log(error,1)
//        })
      this.$ajax.post(this.server +':8084/date/select?areaName=文水县&contentName=占地情况',{}).then(res =>{
