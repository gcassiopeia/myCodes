    ...
    <div>
      ...
      <table id="table-3">
        ...
        <tr v-for="item in tableInfo">
          ...
          <td>{{item.checkSection ? item.checkSection : ''}}</td>
          <td>{{item.waterQuality ? item.waterQuality : ''}}</td>
          <td>{{item.waterState ? item.waterState : ''}}</td>
          <td>{{item.flow ? item.flow : ''}}</td>
          <td>{{item.cOD ? item.cOD : ''}}</td>
          <td>{{item.anDan ? item.anDan : ''}}</td>
        </tr>
      </table>
...
//          console.log(error,1)
//        })
      this.$ajax.post(this.server +':8084/date/select?areaName=文水县&contentName=县域地表水资源质量',{}).then(res =>{
